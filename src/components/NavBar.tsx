...
const navBar = cva(["flex", "items-center", "justify-center", "h-24",
  "w-full", "fixed", "top-0", "left-0", "right-0", "z-20", "pr-4", "bg-gradient-to-b", "from-black/80", "to-black/1", "relative"], {
  variants: {
    intent: {
...
export function NavBar({ intent, className }: NavBarProps) {
  ...
  return <div className={twMerge(navBar({ intent, className }))}>
    ...
    <div className={`lg:hidden ${hamburgerOpen ? "block" : "hidden"} w-full pb-5 ml-5 sm:text-xl text-md bg-[var(--primary-background-color)] text-white absolute top-24 left-0 right-0 z-10`}>
